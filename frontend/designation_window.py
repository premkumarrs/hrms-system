...
class DesignationWindow(QWidget):

    COLUMNS = ["ID", "Designation Title"]
    ...
    def __init__(self, api):
        ...
        self.setWindowTitle("Designations")
        self.resize(700, 520)
    ...
    def build_ui(self):
        ...
        title = QLabel("Designation Management")
        ...
        self.search_input.setPlaceholderText("Search designations...")
        ...
        add_export_buttons(
            toolbar, self, "designations", lambda: self.COLUMNS, self._export_rows
        )

        can_manage = self.api.can("manage_designations")
    ...
    def load_records(self):
        ...
        with loading(self):
            data = self.api.get_designations(search=search or None)

        if show_list_load_error(self, self.api, "designations"):
            return

        self.pager.set_records(data, "designation(s)")
        self.populate_table()
    ...
    def populate_table(self):
        ...
        for row, rec in enumerate(self.pager.page_records):
            ...
            item = QTableWidgetItem(rec.get("title", ""))
    ...
    def add_record(self):
        form = LookupForm("Add Designation", "Title *", parent=self)
        if not form.exec():
            return
        title = form.get_value()
        if not title:
            QMessageBox.warning(self, "Validation", "Designation title is required.")
            return
        ok, error = self.api.create_designation({"title": title})
        if ok:
            self.load_records()
        else:
    ...
    def edit_record(self):
        ...
        if not rec:
            QMessageBox.information(self, "No Selection", "Select a designation.")
            return
        form = LookupForm(
            "Edit Designation", "Title *", initial=rec.get("title", ""), parent=self
        )
        if not form.exec():
            return
        title = form.get_value()
        if not title:
            QMessageBox.warning(self, "Validation", "Designation title is required.")
            return
        ok, error = self.api.update_designation(rec["id"], {"title": title})
        if ok:
            self.load_records()
        else:
            QMessageBox.critical(self, "Save Failed", str(error))
    ...
    def delete_record(self):
        ...
        confirm = QMessageBox.question(
            self,
            "Confirm Delete",
            f"Delete designation '{rec.get('title', '')}'?",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
        )
        if confirm != QMessageBox.StandardButton.Yes:
            return
        ok, error = self.api.delete_designation(rec["id"])
        if ok:
            self.load_records()
        else:
